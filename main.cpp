...
const int MAX_OPERATION_NUMBER = 3;
const int STRETCHING_OPERATION = 0;
const int BTC_OPERATION        = 1;
const int EDGEDETECT_OPERATION = 2;
...
void printMenu() { //{{{
  ...
  cout << "\t(" << BTC_OPERATION << ") Block Truncation Coding \n";
...
void performOperation(unsigned int op) { //{{{
  ...
  switch (op) {
  	case STRETCHING_OPERATION:
  	  ...
  	  break;
  	case BTC_OPERATION:
  	  btc(original, modified, height, width);
  	  break;
